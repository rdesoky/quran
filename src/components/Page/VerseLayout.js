...
const VerseLayout = ({ page: pageIndex, children, pageWidth, versesInfo }) => {
  ...
  const ayaClass = (aya_id) => {
    ...
    className = className.appendWord("Playing", aya_id === player.playingAya);
...
export const HifzSegment = ({ sura, page, pageHeight, versesInfo }) => {
  ...
  const ageClass = age <= 7 ? "GoodHifz" : age <= 14 ? "FairHifz" : "WeakHifz";

  return (
    <div className={`HifzSegment ${ageClass}`} style={{ top, height }}></div>
  );
};
